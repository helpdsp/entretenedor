# Technical Specification — FastTrack AI

> Generado y enriquecido por agente a partir del brief aprobado + PRD v4.3 + clarificaciones del Product Owner.
> Complementar con los artefactos individuales: `data-model.md`, `api-spec.yaml`, `epics.md`, `sprint-plan.md`.

---

## 1. Arquitectura General

FastTrack AI sigue una arquitectura **SPA + BFF-less** donde el frontend React conecta directamente con Supabase como backend unificado (DB + Auth + Storage + Realtime + Edge Functions). No hay un servidor de aplicación intermedio propio.

```
[Browser SPA — Vite + React 18]
         │
         ├─ Supabase JS Client (Auth + DB PostgREST + Storage + Realtime)
         │       ├─ auth.users / profiles (GoTrue)
         │       ├─ PostgREST → RPC PostgreSQL (≥ 25 funciones)
         │       ├─ Storage buckets: avatars / documents / thumbnails
         │       └─ Realtime: tabla notifications (filtro user_id)
         │
         └─ Supabase Edge Functions (Deno)
                 ├─ AI pipeline: upload-document → fragment-document → check-reutilization → generate-atoms → copilot-edit
                 ├─ Gamificación: update-streak, update-leaderboard (cron 60min)
                 ├─ Certificados: issue-certificate, check-cert-outdated
                 ├─ Orgs: create-org-invite, accept-org-invite, waitlist-invite
                 └─ Billing: stripe-checkout, stripe-webhook

[OpenRouter → DeepSeek V3]   ←── invocado por Edge Functions (service role)
[Stripe API]                 ←── stripe-checkout + stripe-webhook
[Resend]                     ←── emails de notificación
```

### Principios arquitectónicos

1. **Supabase es la única fuente de verdad** — No se introducen otras bases de datos ni ORMs.
2. **RLS deny-by-default** — Toda tabla tiene RLS habilitado. El frontend usa `anon key`; las Edge Functions usan `service_role` solo cuando es necesario.
3. **Edge Functions como orquestadores de IA** — Ninguna llamada a OpenRouter o Stripe se hace desde el frontend. Las Edge Functions son el único punto de integración con servicios externos.
4. **Cross-Track Recognition via DB constraint** — El mecanismo de reconocimiento cross-track es el UNIQUE constraint `(user_id, atom_id)` en `user_progress`. No es lógica de aplicación, es integridad referencial.
5. **Leaderboard precalculado** — Los rankings no se calculan en tiempo real. El cron `update-leaderboard` (cada 60 min) escribe en `leaderboard_snapshots`.

---

## 2. Decisiones de Arquitectura (ADRs)

### ADR-01: Wizard usa service_role para rate limiting de IA
**Contexto (OQ-04):** el batch generation del Wizard puede superar las 10 llamadas/hora/usuario si el track tiene > 10 átomos.
**Decisión:** Las Edge Functions del Wizard (`fragment-document`, `generate-atoms`, `copilot-edit`) operan con `service_role` Supabase y no están sujetas al rate limit de usuario (tabla `ai_rate_limits`). El rate limit de usuario aplica solo a llamadas directas del frontend (futuro: herramienta de refinamiento rápido fuera del Wizard).
**Consecuencias:** Los jobs del Wizard se trackean en tabla `ai_jobs` con `triggered_by = 'wizard'` para auditoría y billing futuro.

### ADR-02: Stripe checkout por asignación individual (OQ-01)
**Contexto:** ¿pago por asignación individual o batch mensual?
**Decisión:** Pago inmediato por asignación individual. Flujo: AssignTrackModal selecciona target (Org/Team/Individual) → Edge Function `stripe-checkout` crea Session → Stripe redirige a checkout → `stripe-webhook` recibe `payment_intent.succeeded` → activa `org_track_assignments.status = 'active'`.
**Consecuencias:** Si el admin abandona el checkout, la asignación queda en estado `pending`. Se implementa un cleanup job (función cron diaria) que marca como `expired` las asignaciones `pending` con más de 24h.

### ADR-03: Reutilización de células en Wizard (OQ-03)
**Contexto:** en Paso 4 "Use Existing", ¿se reutiliza la instancia o se crea una copia?
**Decisión:** Reutilizar instancia existente (mismo `cell_id` en `cells`). El RPC `publish_track` inserta en `track_cells` con referencia al `cell_id` existente. Conforme a RN-14: cambios en una célula afectan a todos los tracks que la referencian.
**Consecuencias:** El Creator debe ser advertido en la UI si edita una célula compartida. Se muestra `get_tracks_referencing_cell(cell_id)` antes de confirmar cualquier cambio a título/metadata de célula.

### ADR-04: Cyberpunk skin respeta prefers-reduced-motion (OQ-02)
**Contexto:** los efectos de glow y animaciones holográficas pueden violar WCAG 2.1 AA.
**Decisión:** Cuando `@media (prefers-reduced-motion: reduce)` está activo, el skin Cyberpunk elimina todas las animaciones de movimiento (glow pulso, gradientes animados, transiciones de posición) y las sustituye por `opacity` transitions de 150ms. Los colores y tipografía permanecen.
**Implementación:** Un CSS class `[data-theme="cyberpunk"]` con `@media (prefers-reduced-motion: reduce)` overrides específicos. Testeable con `window.matchMedia('(prefers-reduced-motion: reduce)').matches`.

### ADR-05: Billing en Fase 1 (OQ-05)
**Contexto:** el PRD original listaba pagos como excluidos; las clarificaciones del PO los incluyeron explícitamente.
**Decisión:** El módulo de billing Stripe es parte de Fase 1, limitado al modelo pay-per-seat-per-track. El PRD v4.3 se considera la fuente de verdad; la exclusión original se interpreta como referida a suscripciones/marketplace, no al pago por asignación B2B.
**Consecuencias:** Las Edge Functions `stripe-checkout` y `stripe-webhook` son entregables de Sprint 4.
...
src/
  components/
    atoms/          # Players: QuizPlayer, FlashcardDeck, PlaybookReader, VideoPlayer, TaskAtom
    layout/         # AppShell, Sidebar, TopBar, BottomNav
    ui/             # Shadcn/ui wrappers + custom: Button, Card, Sheet, Popover...
    track/          # NodeGraph, AtomNode, AtomDrawer, TrackCard
    dashboard/      # StreakCounter, WeeklyHeatmap, StatCard, ContinueLearning, MiniLeaderboard
    org/            # OrgDashboard, MemberDetailPanel, TeamDonutChart, AssignTrackModal
    wizard/         # WizardShell, paso 1-7 components, CopilotPanel, DiffViewer
    notifications/  # NotificationBell, NotificationPanel, NotificationItem
    certificates/   # CertificateCard, CertificatePage
    leaderboard/    # PodiumDisplay, LeaderboardTable, GroupCard
    theme/          # ThemeSwitcher, LanguageSwitcher
  hooks/
    useAuth.ts           # Supabase session + profile
    useAtomProgress.ts   # Cross-track progress queries
    useWizardDraft.ts    # localStorage + track_drafts sync
    useNotifications.ts  # Realtime subscription
    useOrg.ts            # Org context + RLS helpers
  ...
  styles/
    themes/          # dark-vibrant.css, light.css, cyberpunk.css (CSS custom properties)
    global.css       # Reset + base layer
  types/             # Zod schemas + TypeScript interfaces
  locales/           # en/ es/ JSON namespaces
```
...
### 4.1 mark_atom_complete (Cross-Track Recognition)

```
User completes atom condition
  → useAtomProgress.markComplete(atomId, trackId)
    → RPC mark_atom_complete(atom_id, track_id)
      → UPSERT user_progress (user_id, atom_id) ON CONFLICT DO NOTHING
        ← returns: {already_completed: bool, track_complete: bool}
      → Edge Function update-streak (async, no-wait)
      → IF track_complete:
          → Edge Function issue-certificate (async)
          → Client shows TrackCompletionModal
```

**Garantías:** el UNIQUE constraint en `user_progress(user_id, atom_id)` hace que el upsert sea idempotente. El cross-track recognition es automático: el átomo ya aparece como completed en todos los tracks que lo contienen.

### 4.2 Content Factory Wizard — AI Pipeline

```
Step 1: upload-document EF
  → multipart upload → Supabase Storage (bucket: documents)
  ← document_url

Step 2: fragment-document EF (service_role)
  → read document from Storage
  → OpenRouter (DeepSeek V3) with fragmentation prompt
  ← { sections: [{ title, atoms: [{ title, type, summary }] }] }
  → INSERT cells (one per section) + ai_jobs record

Step 4: check-reutilization EF
  → pg_trgm similarity query on atoms(title)
  ← [{ existing_atom_id, similarity_score }]

Step 5: generate-atoms EF (service_role, per atom)
  → OpenRouter with type-specific prompt + source excerpt
  ← content JSONB (format depends on atom type)
  → UPDATE ai_jobs item status

Step 6: copilot-edit EF (service_role)
  → current_content + creator message → OpenRouter Copilot prompt
  ← updated content JSONB
  → Client shows diff (green/red highlighting)
```

### 4.3 Stripe Billing — Assign Track

```
Org Admin clicks "Assign Track" in AssignTrackModal
  → POST /stripe-checkout { track_id, org_id, assigned_to_type, assigned_to_id, seat_count }
    → Create Stripe Checkout Session (mode: payment, line_items: price × seat_count)
    ← { checkout_url }
  → Browser redirect to Stripe Checkout
  → User completes payment
  → Stripe sends webhook → stripe-webhook EF
    → verify Stripe signature
    → UPDATE org_track_assignments SET status='active'
    → INSERT notifications (type: track_assigned) for each member
```

### 4.4 Supabase Realtime — Notifications
...
Fallback: si Realtime desconecta (> 30s sin heartbeat), activar polling a `get_notifications` cada 30s.
...
| Capa | Medida |
|------|--------|
| Auth | Supabase GoTrue, refresh tokens, JWT con role claim |
| RLS | deny-by-default en todas las tablas; políticas por rol |
| Edge Functions | Supabase `service_role` solo en EFs de IA y billing |
| Stripe | Verificación de firma HMAC en `stripe-webhook` |
| HTML user-generated | DOMPurify en Task atoms |
| Rate limiting | `ai_rate_limits` tabla (10 calls/user/hour para endpoints frontend) |
| Storage | Bucket `documents` privado (solo accesible por service_role en EFs) |
| Admin routes | RoleGuard: `profiles.role = 'platform_admin'` verificado server-side via RLS |

---

## 6. Performance Targets y Estrategias

| Métrica | Target | Estrategia |
|---------|--------|-----------|
| LCP catálogo | < 2s | GIN index `search_vector`; paginación 20 tracks; lazy-load thumbnails |
| Node Graph render | < 1s | React Flow virtualización; átomos cargados con `get_track_detail` RPC (single query) |
| Edge Functions p95 | < 500ms | Generate-atoms es async (polling); copilot-edit síncrono con timeout 10s |
| Dashboard load | < 1.5s | TanStack Query cache; `get_dashboard_stats` RPC single query indexada |
| Leaderboard | < 500ms | `leaderboard_snapshots` precalculada; índice `(period, period_key, atoms_completed DESC)` |

---

## 7. Testing Strategy

| Nivel | Herramienta | Targets |
|-------|-------------|---------|
| Unit | Vitest | Hooks: `useAtomProgress`, `useWizardDraft`, `useNotifications`; utils; RPC response parsers |
| Component | React Testing Library | AtomPlayers (condition de completado); NodeGraph node states; TrackCard progress |
| Integration | Vitest + Supabase local | `mark_atom_complete` idempotencia; `publish_track` transacción; `assign_track` billing flow |
| E2E smoke | Manual QA | Wizard end-to-end ≤ 60min; Stripe checkout; Leather flow mobile 375px |
| Coverage gate | `npm run gate` | ≥ 78% lines/functions/statements; ≥ 70% branches — **bloqueante en CI** |
| Performance | Lighthouse CI | LCP + FCP en staging post-deploy |

---

## Brief aprobado y PRD de producto (fuentes)
