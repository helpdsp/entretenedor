...
const { writeClarifications, BRIEF_QUESTIONS } = require("../scripts/lib/clarification-store");

function createTempProject() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "vision-brief-preconditions-"));
...
function writeWorkflowState(root, reverseEngineering) {
  ...
  fs.writeFileSync(path.join(root, "planning", "workflow-state.json"), `${JSON.stringify(state, null, 2)}\n`, "utf8");
}
...
test("generate_brief is blocked when reverse engineering is enabled but source code is missing", () => {
  ...
  try {
    ...
    assert.notEqual(result.status, 0);
    assert.match(result.stderr, /generate_brief blocked\./i);
    assert.match(result.stderr, /Reverse engineering mode requires source files/i);
  } finally {
    fs.rmSync(root, { recursive: true, force: true });
  }
});
