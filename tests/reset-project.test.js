...
const RESET_PROJECT_SCRIPT = path.join(__dirname, "..", "scripts", "reset-project.js");
...
test("reset_project removes generated artifacts and preserves refdocs/source code", () => {
  ...
  try {
    fs.writeFileSync(path.join(root, "refdocs", "context.md"), "# user refdoc\n", "utf8");
    fs.writeFileSync(path.join(root, "spec-kit", "input", "brief.md"), "# generated brief\n", "utf8");
    fs.writeFileSync(path.join(root, "spec-kit", "input", "PRD.md"), "# generated PRD\n", "utf8");
    fs.writeFileSync(path.join(root, "source-code", "app.ts"), "export const x = 1;\n", "utf8");
    fs.writeFileSync(path.join(root, "planning", "sprint-index.md"), "# generated\n", "utf8");
    fs.writeFileSync(path.join(root, "planning", "sprints", "sprint-01", "tasks.md"), "# tasks\n", "utf8");
    fs.writeFileSync(path.join(root, ".matrix", "pending-reports.ndjson"), "{}\n", "utf8");
    ...
    assert.equal(fs.existsSync(path.join(root, "spec-kit", "input", "brief.md")), false);
    assert.equal(fs.existsSync(path.join(root, "spec-kit", "input", "PRD.md")), false);
    assert.equal(fs.existsSync(path.join(root, "planning", "sprint-index.md")), false);
    ...
    const workflowStatePath = path.join(root, "planning", "workflow-state.json");
