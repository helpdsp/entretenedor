...
const GENERATE_BRIEF_SCRIPT = path.join(__dirname, "..", "scripts", "generate-brief.js");
const PLAN_SPRINTS_SCRIPT = path.join(__dirname, "..", "scripts", "plan-all-sprints.js");
const CLARIFY_BRIEF_SCRIPT = path.join(__dirname, "..", "scripts", "clarify-brief.js");
...
function runScript(script, root, extraEnv = {}) {
  return spawnSync(process.execPath, [script], {
    cwd: root,
    encoding: "utf8",
    env: { ...process.env, LOCAL_IDE_AI_COMMAND: "", ...extraEnv }
  });
}
...
test("generate_brief is blocked when brief clarifications are missing", () => {
  ...
  try {
    ...
    fs.writeFileSync(path.join(root, "refdocs", "context.md"), "# Context\n", "utf8");
...
test("nextAction is clarify_brief when brief clarifications are missing", () => {
  const root = createTempProject();
  try {
    fs.writeFileSync(path.join(root, "refdocs", "requirements.md"), "# refdoc\n", "utf8");
    writeWorkflowState(root);
    const summary = buildStatusSummary(root);
    assert.equal(summary.nextAction.name, "clarify_brief");
  } finally {
    fs.rmSync(root, { recursive: true, force: true });
  }
});
...
test("nextAction is clarify_sprints when sprints clarifications are missing", () => {
  const root = createTempProject();
  try {
    writeWorkflowState(root, {
      status: "spec_approved",
    ...
    const summary = buildStatusSummary(root);
    assert.equal(summary.nextAction.name, "clarify_sprints");
  } finally {
    fs.rmSync(root, { recursive: true, force: true });
  }
});
...
test("buildStatusSummary includes clarifications status", () => {
  ...
  try {
    ...
    assert.ok(Object.prototype.hasOwnProperty.call(summary.clarifications, "brief"));
    assert.ok(Object.prototype.hasOwnProperty.call(summary.clarifications, "sprints"));
  } finally {
    fs.rmSync(root, { recursive: true, force: true });
  }
});
...
test("clarify_brief --answers-file saves valid clarifications", () => {
  ...
  try {
    ...
    const result = spawnSync(process.execPath, [CLARIFY_BRIEF_SCRIPT, "--answers-file", answersFile], {
      cwd: root,
      encoding: "utf8",
      env: { ...process.env }
    });
...
test("clarify_brief --answers-file fails when a question is missing", () => {
  ...
  try {
    const answersFile = path.join(root, "answers.json");
    fs.writeFileSync(answersFile, JSON.stringify({ brief_tone: "Ejecutivo" }), "utf8");
