...
const {
  isUpstreamCloned: isSpecKitCloned,
  gitAvailable,
  readMeta: readSpecKitMeta,
} = require('./update-spec-kit');
const {
  isUpstreamCloned: isAgencyAgentsCloned,
  readMeta: readAgencyMeta,
} = require('./update-agency-agents');
...
function toStoredDir(root, inputValue, fallbackRelative) {
  ...
  const absolute = path.isAbsolute(rawValue)
    ? rawValue
    : path.resolve(path.join(root, rawValue));
...
function runScript(root, scriptName, label) {
  ...
  if (result.status !== 0) {
    console.warn(
      `Warning: No se pudo actualizar ${label}. Continuando sin actualizacion.`,
    );
  }
}
...
async function main() {
  ...
  const preferredStatusPort = Number(
    args.port || process.env.STATUS_WEB_PORT || String(DEFAULT_PORT),
  );
  ...
  const promptDisabled =
    String(args['no-prompt'] || 'false').toLowerCase() === 'true';
  ...
  let updateSpecKit =
    specKitArg !== undefined
      ? ['si', 'sí', 'yes', 'y', '1', 'true'].includes(
          String(specKitArg).trim().toLowerCase(),
        )
      : null;
  ...
  if (updateSpecKit === null && !promptDisabled && process.stdin.isTTY) {
    if (gitOk) {
      ...
      const hint = meta
        ? ` (ultima actualizacion: ${meta.updatedAt.slice(0, 10)})`
        : '';
  ...
  let updateAgency =
    agencyArg !== undefined
      ? ['si', 'sí', 'yes', 'y', '1', 'true'].includes(
          String(agencyArg).trim().toLowerCase(),
        )
      : null;
  ...
  if (reverseEngineering === true) {
    ...
    console.log(
      `Required before generate_brief: add source code files to ${sourceDir} and add at least 1 refdoc in ${referencesDir}.`,
    );
  } else if (reverseEngineering === false) {
    console.log('Reverse engineering: NO.');
    console.log(
      `Required before generate_brief: add at least 1 refdoc in ${referencesDir}.`,
    );
  } else {
  ...
  console.log(
    'First instruction: run `generate_brief` when preconditions are ready.',
  );
  ...
  try {
    ...
    if (statusPort !== preferredStatusPort) {
      console.log(
        `Status web: port ${preferredStatusPort} busy, using ${statusPort} instead.`,
      );
    }
  } catch (error) {
    console.warn(
      `Status web: ${error.message} — skipping local status server and browser.`,
    );
  }
  ...
  if (statusPort !== null) {
    const server = spawn(
      process.execPath,
      [statusScript, '--port', String(statusPort), '--strict-port'],
      {
        cwd: root,
        stdio: 'inherit',
      },
