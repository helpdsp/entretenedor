...
function getLocalChecklist(root, briefPreconditions) {
  ...
  const refsDir =
    path.relative(root, briefPreconditions.referencesDir).replace(/\\/g, '/') ||
    briefPreconditions.referencesDir;
  const sourceDir =
    path.relative(root, briefPreconditions.sourceDir).replace(/\\/g, '/') ||
    briefPreconditions.sourceDir;
  ...
  return [
    ...
    },
    {
      ...
      label: briefPreconditions.sourceRequired
        ...
        !briefPreconditions.sourceRequired ||
        briefPreconditions.sourceCount > 0,
    ...
    },
    {
      ...
      label: 'Local AI command configured',
        ...
        hasValue(localAiCommand) &&
        localAiCommand.includes('{{INPUT_FILE}}') &&
        localAiCommand.includes('{{OUTPUT_FILE}}'),
...
function computeNextCommand(workflow, briefPreconditions, root) {
  ...
  if (!workflow.sprints.generated) {
    const sprintsClarificationsComplete = isClarificationsComplete(
      root,
      'sprints',
    );
    return buildNextAction(
      'generate_sprints',
      [],
      sprintsClarificationsComplete
        ? 'Generate the full sprint plan.'
        : 'Generate the full sprint plan. Run clarify_sprints first to answer sprint planning questions.',
      !sprintsClarificationsComplete,
    );
  }
  if (workflow.sprints.active) {
    ...
    const nextNum = a + 1;
    const hasNextPlanned =
      workflow.sprints.total > 0 && nextNum <= workflow.sprints.total;

    if (report.error === 'missing_file' || report.error === 'no_rows') {
      const rel =
        path.relative(root, report.path).replace(/\\/g, '/') || report.path;
      return buildNextAction(
        'continue_sprint',
        ['--sprint', String(a)],
    ...
    if (
      report.ok &&
      workflow.sprints.total > 0 &&
      nextNum > workflow.sprints.total
    ) {
      return buildNextAction(
        'planning:finalize',
        [],
  ...
  return buildNextAction(
    'planning:finalize',
    [],
    'Finalize planning package and generate ZIP.',
  );
}

function computeBlockers(workflow, matrix, briefPreconditions, root) {
  const blockers = [];
  ...
  // Check sprints clarifications when sprints are not yet generated
  if (
    !workflow.sprints.generated &&
    !isClarificationsComplete(root, 'sprints')
  ) {
    blockers.push({
      code: 'missing_sprints_clarifications',
      message: 'Sprints clarifications are incomplete or missing.',
      unblock: 'Run `clarify_sprints` to answer the sprint planning questions.',
      blocking: true,
    });
  }
  ...
  if (matrix.connectionStatus === MATRIX_CONNECTION_STATES.PENDING_AUTH) {
    blockers.push({
      ...
      unblock:
        'Run `npm run matrix:authorize -- --authorized-by "<your-name>"` or continue in local-only mode.',
      blocking: false,
    });
  }
  ...
  if (matrix.connectionStatus === MATRIX_CONNECTION_STATES.DENIED) {
    blockers.push({
      ...
      unblock:
        'Review MATRIX_API_KEY/projectId/workspaceId and retry authorization.',
      blocking: false,
    });
  }

  if (
    matrix.connectionStatus === MATRIX_CONNECTION_STATES.CONNECTED &&
    !matrix.apiKey
  ) {
    blockers.push({
      code: 'matrix_missing_api_key',
      message:
        'Matrix is marked as connected but MATRIX_API_KEY is not configured.',
...
function buildStatusSummary(root) {
  ...
  const blockers = computeBlockers(workflow, matrix, briefPreconditions, root);
  ...
  return {
    ...
    briefPreconditions: {
      ...
      referencesDir:
        path
          .relative(root, briefPreconditions.referencesDir)
          .replace(/\\/g, '/') || briefPreconditions.referencesDir,
      sourceDir:
        path.relative(root, briefPreconditions.sourceDir).replace(/\\/g, '/') ||
        briefPreconditions.sourceDir,
