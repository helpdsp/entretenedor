---
description: VISION — generate_brief (generar brief del proyecto)
---

Genera el brief del proyecto a partir de los refdocs disponibles (y source-code si es reverse engineering).

## Instrucciones

El brief lo redacta **el agente** (tú) leyendo directamente los archivos de `refdocs/` y `source-code/`.
El script `node scripts/generate-brief.js` **solo registra el estado** del workflow cuando se usa el flag `--agent-written`; no genera contenido por sí mismo en modo local.
...
Si `reverseEngineering` es `null`, bloquear: pedir al usuario que ejecute `init` primero.
...
Si detectas ambigüedades críticas (objetivo de negocio poco claro, audiencia no definida, scope contradictorio), invoca primero: `clarify_brief`
...
3. **Si existe `planning/clarifications/brief.json`**: leerlo e incorporar las respuestas como contexto adicional.

---

### Paso 4 — Escribir el brief

Crea o sobreescribe `spec-kit/input/brief.md` con un brief ejecutivo de alta calidad basado en el contenido leído. El brief debe tener **estas secciones obligatorias**:
...
- En modo **reverse engineering**: describir el sistema existente (arquitectura, módulos, entidades, flujos principales) como "estado actual" y dejar espacio para objetivos de refactoring/extensión.
...
Una vez escrito `spec-kit/input/brief.md`, ejecuta el script con el flag `--agent-written` para registrar el evento en el workflow **sin sobreescribir** el archivo:

```bash
node scripts/generate-brief.js --agent-written
```

Si el script falla con un error de precondiciones (e.g., `reverse_engineering_not_defined`), resuelve el bloqueante antes de continuar.
